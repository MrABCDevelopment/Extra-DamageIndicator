package me.dreamdevs.damageindicator.versions.v1_8_R3;
...
public class HologramIndicator implements IHologramIndicator {
	...
	public void showHologram(LivingEntity target, HealthChangeType healthChangeType, double health) {
		...
		EntityArmorStand entityArmorStand = new EntityArmorStand(craftWorld.getHandle());
	...
	public void showActionBar(Player player, LivingEntity target, double health) {
		...
		double percentage = Math.floor((target.getHealth()-health) / target.getMaxHealth()*100);
		for(int x = 0; x<10; x++) {
			if(x < (percentage/10)) {
		...
		PacketPlayOutChat packet = new PacketPlayOutChat(IChatBaseComponent.ChatSerializer.a("{\"text\":\"" + ColourUtil.colorize(stringBuilder.toString()) + "\"}"), (byte) 2);
