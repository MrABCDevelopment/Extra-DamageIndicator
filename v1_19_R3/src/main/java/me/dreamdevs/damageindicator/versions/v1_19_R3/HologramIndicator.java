package me.dreamdevs.damageindicator.versions.v1_19_R3;
...
public class HologramIndicator implements IHologramIndicator {
	...
	public void showHologram(LivingEntity target, HealthChangeType healthChangeType, double health) {
		...
		PacketPlayOutEntityMetadata packetPlayOutEntityMetadata = new PacketPlayOutEntityMetadata(entityArmorStand.af(), entityArmorStand.aj().b());
		...
		Bukkit.getScheduler().runTaskLaterAsynchronously(DamageIndicatorApi.plugin, () -> {
			...
			Bukkit.getOnlinePlayers().forEach(player -> ((CraftPlayer)player).getHandle().b.a(packetPlayOutEntityDestroy, null));
	...
	public void showActionBar(Player player, LivingEntity target, double health) {
		...
		((CraftPlayer)player).getHandle().b.a(clientboundSystemChatPacket, null);
